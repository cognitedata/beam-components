...
mvn compile exec:java -D exec.mainClass=com.cognite.sa.beam.bq.CdfEventsBQ -D exec.args="--cdfSecret=$cdfSecret --cdfHost=$cdfHost --bqTempStorage=$bqTempStorage --outputMainTable=$outputMainTable --fullRead=$fullRead --project=$gcpProject --runner=DataFlowRunner --gcpTempLocation=$gcpBucketPrefix-test/temp --stagingLocation=$gcpBucketPrefix-test/stage/cdf-events-bq --region=europe-west1 --experiments=shuffle_mode=service --numWorkers=10 --maxNumWorkers=10 --experiments=enable_stackdriver_agent_metrics --workerMachineType=e2-standard-2"