...
# The job config file
$jobConfigFile = 'gs://beam-component/config/replicate/job-config-ts-replication.toml'

mvn compile exec:java -D exec.mainClass=com.cognite.sa.beam.replicate.ReplicateTs -D exec.args="--cdfSourceConfigFile=$cdfSourceConfigFile --cdfTargetConfigFile=$cdfTargetConfigFile --jobConfigFile=$jobConfigFile --project=cognite-sa-sandbox --runner=DataFlowRunner --gcpTempLocation=gs://beam-component/temp --stagingLocation=gs://beam-component/stage/replicate-ts --region=europe-west1 --experiments=shuffle_mode=service --numWorkers=4 --maxNumWorkers=4 --workerMachineType=n2-standard-2"
